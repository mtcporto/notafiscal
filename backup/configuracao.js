...
function carregarConfiguracoes() {
  ...
  if (config.webservice) {
    ...
    document.getElementById('urlWebservice').value = config.webservice.url || 'https://serem-hml.joaopessoa.pb.gov.br/notafiscal-abrasfv203-ws/NotaFiscalSoap';
  ...
  try {
    if (tipo === 'A1') {
      ...
      const resultadoValidacao = await validarCertificadoA1(arquivo, senha);
        ...
        if (arquivo.name === 'pixelvivo.pfx') {
          autoPreencherDadosPixelVivo();
        }
...
async function extrairInformacoesCertificado(nomeArquivo) {
  ...
  const certificados = {
    ...
    'pixelvivo.pfx': {
      nomeTitular: 'PIXEL VIVO SOLUCOES WEB LTDA',
      cpfCnpj: '15198135000180',
      validadeAte: '2025-12-31T23:59:59.000Z',
      emissor: 'AC Digital',
      serialNumber: 'PV:2025:REAL'
    }
  };
